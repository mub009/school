...
// get data with paging
    ...
    public function _send_email() {
        ...
        if($this->input->post('email')){         
            ...
            $school_id = $this->input->post('school_id') ? $this->input->post('school_id') : 0;
            if($school_id){            
                $setting = $this->db->get_where('schools', array('status' => 1, 'id'=>$school_id))->row();    
                $from_email = $setting->email;
                $from_name = $setting->school_name;
            }else{
                $setting = $this->db->get_where('global_setting', array('status'=>1))->row();
            ...
            $message .= '<br/><br/>';
            $message .= 'Following is your ' . $from_name . ' web Application login credentials.<br/>';
            $message .= '<br/><br/>';
            ...
            $message .= 'If you are right person, Plesae ignore this email.<br/><br/>';
    ...
    public function _send_sms(){
        ...
        if($this->input->post('phone')){ 
            ...
            $sms_gateway = $this->db->get_where('sms_settings', array('status' => 1, 'school_id'=>$this->input->post('school_id')))->row(); 
            $gateway = '';
            ...
            if($this->sms_gateway($gateway)){
                ...
                $message = 'Hi, '.$this->input->post('name'). ' Username: '.$username. ' Password: '.$password. ' for '.$setting->school_name;
